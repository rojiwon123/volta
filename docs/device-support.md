...
### 4-2. 효과 검증(행동 기반) + 런타임 강등
키가 존재해(probe) write까지 됐더라도 **실제로 거동이 바뀌는지**는 별개다. 제어 적용 후 배터리/전력 거동을 관찰해 효과를 확인하고, 안 먹으면 런타임에 제어를 끈다.

- **무엇을 보나(거동)**: write-only 제어 키(CHTE/CHIE)는 값 read-back을 신뢰 못 하므로 **거동**으로 본다.
...
- **정착/샘플링**: SMC/OS 반영 시간차가 있어 **정착 지연 후 수 틱 샘플링**해 판정(즉시 판정 금지). 초기 샘플은 버리고, 최소 결정 샘플 수 미만이거나 조건 불충족·노이즈면 `inconclusive`(강등 안 함 — false-downgrade 방지).
- **헬퍼 write 성공 시에만 검증(오탐 방지)**: 효과 검증은 **헬퍼로 SMC write가 실제 수행(성공)된 경우에만** 돈다. 헬퍼가 미설치/미연결/연결 실패면 `helper.set*()`가 false(write 미수행) → 검증·강등을 **하지 않는다**. (유료 팀 계정/헬퍼 없이 write 자체가 안 일어나는 정상 상태를 "효과 미관찰"로 오판하지 않게.) 게이트: `ControlEffectVerifier.shouldVerify(writePerformed:controlSupported:)`.
- **헬퍼 부재 ≠ `.ineffective`**: 헬퍼 미설치/미연결은 `.ineffective`(기기 제어 불가)와 구분해 **`HelperStatusView`(헬퍼 상태 안내)** 로만 표시한다. 헬퍼가 enabled가 아니면 잘못 남은 런타임 강등도 자동 해제(`DeviceSupportResult.clearedIfControlWriteUnavailable`). `.ineffective`는 오직 "헬퍼가 실제로 write했는데도 거동이 안 바뀐" 진짜 케이스에서만.
- **런타임 강등(fail-safe)**: `notObserved`면 `DeviceSupportResult.supported → .ineffective(reason:)`로 내려가고, **안전 상태로 수렴**(충전 허용·어댑터 정상·억제 해제·강제방전/외출준비 해제) 후 제어를 비활성한다. (런타임 상태 — 저장 안 함, 앱 재시작 시 base로 복귀.)
- 코드: 판정은 순수 함수 `ControlEffectVerifier.judge(intent:before:after:policy:)`(단위 테스트), `SMCService.verifyControlEffect(...)`가 이를 호출, 트리거 게이트/샘플링/강등 응답은 `BatteryMonitor`(헬퍼 write 성공 확인·정착 지연·샘플 수집·`applyingControlEffect`·안전 수렴).
- ⚠️ **판정은 "주어진 샘플상"일 뿐 실기 효과 검증이 아니다.** 실제 효과/강등이 맞게 도는지는 서명된 root 헬퍼·실기에서만 확정(유료 팀 계정 후). `verifiedOnHardware` 승격도 그때.
...
- 제어를 단계별로 직접 적용→관찰→복원하고, 결과를 `SelfTest.resolvedSupport(base:results:)`(순수)로 검증상태에 반영한다.
- **충전 억제(기반 제어) `notWorking` → `.ineffective` 강등**(4-2와 동일 의미, `applyingControlEffect(.notObserved)`). **모든 단계 `working` → `mappedUnverified`→`verifiedOnHardware` 승격**(`promotedToVerified()`). 강제 방전 단독 실패는 강등하지 않고 보고로만.
- 판정 불가(어댑터 미연결/충전 아님/노이즈)는 강등·승격 둘 다 하지 않는다(불확실 보수).
- 런타임 상태(저장 안 함). 코드: 순수 로직 `VoltaCore.SelfTest`(단위 테스트 `SelfTestTests`), 라이브 오케스트레이션 `BatteryMonitor.runSelfTest`.
